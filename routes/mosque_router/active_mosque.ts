...
router.get("/mosques_search", async (req: Request, res: Response) => {
  try {
    ...
    const condetion2 = typeof Lan === "string" && !Number.isNaN(Lan);
    const condetion3 = typeof Log === "string" && !Number.isNaN(Log);
    if (condetion1 && !condetion2 && !condetion3) {
      const mosques = await Mosque.find({
        "MosqueProps.MosqueName": { $regex: new RegExp(name, "i") },
      }).lean();

      res.status(200).json(mosques.map((e) => e.MosqueProps));
    } else if (!condetion1 && condetion2 && condetion3) {
      const mosques = await Mosque.find({}).lean();
      const [Lant, Long] = [Lan, Log].map((e, i) => Number(e));
      const Nearby_mosques = mosques
        .filter(
          (e) =>
            !Number.isNaN(e.MosqueProps?.Lat) &&
            !Number.isNaN(e.MosqueProps?.Lon)
        )
        .map((e) => {
          const dilatX = (Lant! -  e.MosqueProps!.Lat!)*111
          const dilatY = (Long! -  e.MosqueProps!.Lon!)*111.32*Math.cos(Lant!*(Math.PI/180))

          return {...e.MosqueProps,Distance:(dilatX**2 + dilatY**2)**0.5}
        }).filter(e=>e.Distance<=1.3)
        res.status(200).json(Nearby_mosques);
      }
...
router.post("/set_active", async (req: Request, res: Response) => {
  try {
    ...
    const { Lon, Lat, MosqueName, Country, Region, City } =
      req.body as reqest_data;
    const token = req.headers.authorization?.split(" ")[1];
    ...
    if (mosuqe) {
      ...
      mosuqe.MosqueProps = {
        ...mosqueProps,
        MosqueId: mosuqe.MosqueProps?.MosqueId ?? new_mosque_id,
      };
      await mosuqe.save();
